<?php defined('THINK_PATH') or die("Access denied!"); ?>
<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8" />
    ...
    <div id="wrap" class="wrap">
        <div class="head row">
            <a href="__APP__/home/area/getprovince">
            <div class="col-xs-3" style="overflow: hidden;white-space: nowrap;">
                <?php 
                $area ='';
                if($_SESSION['town_name']!=''){
                    $area = $_SESSION['town_name'];
                    }else{
                        if($_SESSION['dist_name']!=''){
                            $area = $_SESSION['dist_name'];
                        }else{
                            if($_SESSION['city_name']){
                                $area = $_SESSION['city_name'];
                            }else{
                                if($_SESSION['prov_name']){
                                    $area = $_SESSION['prov_name'];
                                }else{
                                    $area = '选择城市';
                                }
                            }
                    ...
                    echo mb_strlen($area,'utf8')>4?mb_substr($area,0,4,'utf8'):$area;
                ?>  
        ...
        <div id="nav_bar" class="section" style="margin-top:-1px">
            <div class="section_body row">
                <foreach name="typeList" item="vo">
                    <div  class="col-xs-3">
                        <a href="__APP__/home/goods/index/type1/{$vo['id']}">
                            <img src="<?php echo C("HOME_PATH") ?>/public/uploads/type/{$vo['id']}" class="nav_icon" />
                        </a>
                        <p style="margin-top:10px">{$vo['type_name']}</p>
                    </div>
                </foreach>
             </div>
        </div>
  ...
  $(function(){
        var get_recomment_brands=function(){
            ...
            $.ajax({
                type: "get",
                dataType: "json",
                url: '__APP__/home/goods/get_recommend_brands',
                    ...
                    if (data != "") {
                        ...
                        for(var j=0;j<data.length;j++){
                            ...
                            for(var i=0;i<data[j].length;i++){
                                rbs+="<a href='__APP__/home/goods/index/brand/"+data[j][i].id+"'>";
        ...
        var get_recommend_goods =function(){
            ...
            $.ajax({
                type: "get",
                dataType: "json",
                url: '__APP__/home/goods/get_recommend_goods',
                success: function (data) {
                    if (data != "") {
                        //遍历对象数组，填充列表
                        console.log(data);
